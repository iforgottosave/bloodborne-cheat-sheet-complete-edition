var profilesKey = 'bloodborne_profiles';
...
(function($) {
    ...
    jQuery(document).ready(function($) {
        ...
        $('#NG\\+ModalYes').click(function(event) {
            event.preventDefault();
            if (!confirm('Are you sure you wish to begin the next hunt?')) {
                return;
            }
            ...
            if (profiles[profilesKey][profiles.current].hunt < 3) {
                profiles[profilesKey][profiles.current].hunt++;
            }
            $.jStorage.set(profilesKey, profiles);
            populateChecklists();
            restoreState(profiles.current);
        ...
        */
        ...
        $('[data-ng-toggle]').change(function() {
            var hunt = $(this).data('ng-toggle');

            profiles[profilesKey][profiles.current].hunt = +hunt
            $.jStorage.set(profilesKey, profiles);
    ...
    function initializeProfile(profile_name) {
        ...
        if (!('hunt' in profiles[profilesKey][profile_name]))
            profiles[profilesKey][profile_name].hunt = 1;
    ...
    function restoreState(profile_name) {
        ...
        $('[data-ng-toggle="' + profiles[profilesKey][profile_name].hunt + '"]').click().change();
    ...
    function canFilter(entry) {
        ...
        for (var i = 0; i < classList.length; i++) {
            // Hide(h) or show(s) entries based on hunt number
            if ((classList[i].match(/^h_ng\+*$/) && classList[i].match(/^h_ng(\+*)$/)[1].length < profiles[profilesKey][profiles.current].hunt) ||
               (classList[i].match(/^s_ng\+*$/) && classList[i].match(/^s_ng(\+*)$/)[1].length >= profiles[profilesKey][profiles.current].hunt)) {
                return true;
            }
        }
        var foundMatch = 0;
